...
private:
    ...
    secure_queue<T> to_clients;
    secure_queue<T> to_server;
    ...
    std::map<int, connection<message> *> connects;
};

template <typename T>
void server<T>::push(const T &msg) {
    to_clients.push(msg);
}
...
template <typename T>
void server<T>::lock_to_cli() {
    to_clients.lock();
}

template <typename T>
void server<T>::unlock_to_cli() {
    to_clients.unlock();
}

template <typename T>
void server<T>::lock_to_serv() {
    to_server.lock();
}

template <typename T>
void server<T>::unlock_to_serv() {
    to_server.unlock();
}

template <typename T>
void * find_client(server<T>* arg) {
    fooooooooooooooooooooooooooooooooooo();
    server<message> *srv = static_cast<server<message>*>(arg);
    ...
    while(1) {
        ...
        auto *conn = new connection<message>(new_sock_fd, current_new_id);
        auto p = std::pair<int, connection<message>*>(1, conn);
...
template <typename T>
void * distribution(server<T>* arg) {
    server<message> *srv = static_cast<server<message>*>(arg);
    while(1) {
        srv->connects_mutex.lock();
        for(std::map<int, connection<message>*>::iterator i = srv->connects.begin(); i != srv->connects.end(); i++) {
...
server<T>::server() {
    ...
    pthread_create(&new_client_listen, NULL, find_client, this);
    pthread_create(&distributor, NULL, distribution, this);
}
